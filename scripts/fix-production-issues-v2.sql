-- 本番環境の問題を修正する統合スクリプト（修正版）
-- 実行日: 2025-07-28

-- ==========================================
-- 1. get_dashboard_stats関数の型不一致を修正（実際のカラム名を使用）
-- ==========================================
...
BEGIN
  ...
  SELECT
    ...
    -- アクティブなキャスト数（is_activeを使用）
    (SELECT COUNT(*)
     FROM staffs s
     JOIN casts_profile cp ON s.id = cp.staff_id
     WHERE s.is_active = true
       AND cp.is_active = true)::NUMERIC AS active_cast_count,
    
    -- アクティブなテーブル数（is_activeを使用）
    (SELECT COUNT(*)
     FROM tables
     WHERE is_active = true)::NUMERIC AS active_table_count,
    
    -- 在庫不足商品数（is_activeを使用）
    (SELECT COUNT(*)
     FROM products
     WHERE stock_quantity <= low_stock_threshold
       AND is_active = true)::NUMERIC AS low_stock_count,
