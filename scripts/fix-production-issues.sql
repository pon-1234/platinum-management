...
-- ==========================================
-- 1. get_dashboard_stats関数の型不一致を修正
-- ==========================================
  ...
  SELECT
    ...
    (SELECT COUNT(*)
     ...
     WHERE s.is_available = true
       AND cp.is_available = true)::NUMERIC AS active_cast_count,
    
    -- アクティブなテーブル数
    (SELECT COUNT(*)
     FROM tables
     WHERE is_available = true)::NUMERIC AS active_table_count,
    
    -- 在庫不足商品数
    (SELECT COUNT(*)
     FROM products
     WHERE stock_quantity <= low_stock_threshold
       AND is_available = true)::NUMERIC AS low_stock_count,
