...
BEGIN
  ...
  SELECT
    ...
    (SELECT COALESCE(SUM(oi.total_price), 0)
     ...
     JOIN visits v ON oi.visit_id = v.id
     WHERE v.check_in_at::date = report_date
       AND v.status = 'completed') AS today_sales,
    
    -- 本日の来店数
    (SELECT COUNT(*)
     FROM visits
     WHERE check_in_at::date = report_date
       AND status = 'completed') AS today_visits,
    
    -- 本日の顧客数（重複なし）
    (SELECT COUNT(DISTINCT customer_id)
     FROM visits
     WHERE check_in_at::date = report_date) AS today_customers,
    
    -- 本日の新規顧客数
    (SELECT COUNT(*)
     FROM customers
     WHERE created_at::date = report_date
       AND is_deleted = false) AS today_new_customers,
    
    -- アクティブなキャスト数（本日出勤中）
    (SELECT COUNT(DISTINCT cp.id)
     FROM casts_profile cp
     JOIN attendance_records ar ON ar.staff_id = cp.staff_id
     WHERE ar.attendance_date = report_date
       AND ar.status = 'present'
       AND cp.is_active = true) AS active_cast_count,
    
    -- 利用可能なテーブル数
    (SELECT COUNT(*)
     FROM tables
     WHERE is_available = true) AS active_table_count,
    ...
    (SELECT COUNT(*)
     ...
     WHERE reservation_date = report_date
       AND status = 'pending') AS pending_reservations;
END;
$$ LANGUAGE plpgsql;
